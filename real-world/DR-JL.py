"""
DR Estimator with Joint Learning approach.
"""
import pickle
import numpy as np
...
prediction_op = tf.train.AdamOptimizer(lr).minimize(mf.loss_jl)
imputation_op = tf.train.AdamOptimizer(lr_il).minimize(mf.loss_il_jl)
sess = tf.Session()
...
while early_stop < 5:
    ...
    dcg = dcg_at_k(obj, prediction, eval=True)
    recall = recall_at_k(obj, prediction, eval=True)
