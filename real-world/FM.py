"""
Backbone: factorization machine (FM).
We only take user-id and item-id as the input feature.
"""
...
class drFM:
    def __init__(self, user_num, item_num, embedding_size, l2_reg_lambda, l2_reg_lambda_il):
        ...
        self.squared_loss = tf.square(self.prediction_il - tf.stop_gradient(tf.nn.sigmoid_cross_entropy_with_logits(logits=self.prediction, labels=self.y)))
        ...
        self.loss_il_jl = tf.reduce_mean(self.squared_loss) + l2_reg_lambda_il * self.l2_regularization_il
        ...
        self.dr_loss_jl = tf.multiply(self.error-tf.stop_gradient(self.prediction_il), tf.div(self.o, self.p)) + tf.stop_gradient(self.prediction_il)
        self.l2_regularization = tf.nn.l2_loss(self.user_embedding) + tf.nn.l2_loss(self.item_embedding)
        self.l2_regularization += tf.nn.l2_loss(self.user_bias) + tf.nn.l2_loss(self.item_bias)
        self.l2_regularization += tf.nn.l2_loss(self.global_bias)
        self.loss = tf.reduce_mean(self.dr_loss) + l2_reg_lambda * self.l2_regularization
        self.loss_jl = tf.reduce_mean(self.dr_loss_jl) + l2_reg_lambda * self.l2_regularization
